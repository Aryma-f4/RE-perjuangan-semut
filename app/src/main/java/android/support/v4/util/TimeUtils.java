...
public class TimeUtils {
    ...
    private static int formatDurationLocked(long duration, int fieldLen) {
        ...
        int seconds2 = (int) Math.floor(duration / 1000);
        int days = 0;
        int hours = 0;
        int minutes = 0;
        ...
        if (seconds2 > SECONDS_PER_HOUR) {
            hours = seconds2 / SECONDS_PER_HOUR;
            seconds2 -= hours * SECONDS_PER_HOUR;
        }
        if (seconds2 > SECONDS_PER_MINUTE) {
            minutes = seconds2 / SECONDS_PER_MINUTE;
            seconds = seconds2 - (minutes * SECONDS_PER_MINUTE);
        } else {
            seconds = seconds2;
        }
        int pos = 0;
