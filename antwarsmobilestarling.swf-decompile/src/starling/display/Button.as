package starling.display
{
   ...
   public class Button extends DisplayObjectContainer
   {
      ...
      private function onTouch(param1:TouchEvent) : void
      {
         ...
         else if(_loc2_.phase == "moved" && mState == "down")
         {
      ...
      {
         mState = param1;
         mContents.scaleX = mContents.scaleY = 1;
         switch(mState)
         {
            case "down":
               setStateTexture(mDownState);
               mContents.scaleX = mContents.scaleY = scaleWhenDown;
               mContents.x = (1 - scaleWhenDown) / 2 * mBody.width;
               mContents.y = (1 - scaleWhenDown) / 2 * mBody.height;
               break;
            case "up":
